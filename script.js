/* =====================================================
   RESUME INSIGHT - FINAL JAVASCRIPT
===================================================== */
...
function getName(text) {
    ...
    /*
       ...
       PARV KHARE Frontend Developer | AI Enthusiast
       ABOUT ME
       Computer Science...
    */
    ...
    for (const line of lines) {
        ...
        const match = line.match(
            /^([A-Z][A-Z .'-]{2,40})\s+(?:Frontend Developer|AI Enthusiast)/i
        );
    ...
    /*
       Fallback:
       Look for a line beginning with a likely name
    */
...
function getEducation(text) {
    ...
    */
    ...
    const srmMatch = normalized.match(
        /SRM Institute of Science and Technology.*?(?:2025\s*[-–]\s*2029)/i
    );

    if (srmMatch) {

        let education = srmMatch[0]
            .replace(/\s+/g, " ")
            .trim();
        ...
        */
        ...
        for (const word of stopWords) {
            ...
            const index =
                education.toUpperCase().indexOf(word);
    ...
    */
    ...
    const genericMatch = normalized.match(
        /(?:SRM Institute of Science and Technology|B\.?\s*Tech.*?Computer Science.*?)(?:2025\s*[-–]\s*2029)?/i
    );
...
function getProjects(text) {
    ...
    if (
        /carenetra|project|projects|patient monitoring|web application/i
            .test(text)
    ) {
...
function getCertifications(text) {
    ...
    if (
        /certification|certifications|certificate|certified/i
            .test(text)
    ) {
...
function getAchievements(text) {
    ...
    if (
        /achievement|achievements|award|winner|hackathon|scholarship/i
            .test(text)
    ) {
...
function calculateScores(data) {
    ...
    const certificationsScore =
        data.certifications === "No data available"
            ? 0
            : 10;


    const achievementsScore =
        data.achievements === "No data available"
            ? 0
            : 10;


    const qualityScore =
        data.text.length > 1000 ? 13 :
        data.text.length > 500 ? 10 :
        7;
...
function updateUI(data) {
    ...
    document.getElementById("resultEducation").textContent =
        data.education;
    ...
    document.getElementById("resultCertifications").textContent =
        data.certifications;

    document.getElementById("resultAchievements").textContent =
        data.achievements;
    ...
    document.getElementById("scoreStatus").textContent =
        getScoreStatus(data.scores.overall);


    document.getElementById("scoreMessage").textContent =
        getScoreMessage(data.scores.overall);


    /* CATEGORY SCORES */

    document.getElementById("skillsScore").textContent =
        `${data.scores.skills}/20`;

    document.getElementById("projectsScore").textContent =
        `${data.scores.projects}/20`;

    document.getElementById("certificationsScore").textContent =
        `${data.scores.certifications}/15`;

    document.getElementById("achievementsScore").textContent =
        `${data.scores.achievements}/15`;

    document.getElementById("qualityScore").textContent =
        `${data.scores.quality}/15`;
    ...
    document.getElementById("skillsFeedback").textContent =
        data.scores.skills >= 15
            ? "Good technical skill coverage."
            : "Add more relevant technical skills.";

    document.getElementById("projectsFeedback").textContent =
        data.scores.projects >= 15
            ? "Projects are clearly present."
            : "Add more detailed projects.";

    document.getElementById("certificationsFeedback").textContent =
        data.scores.certifications > 0
            ? "Certifications detected."
            : "Consider adding relevant certifications.";

    document.getElementById("achievementsFeedback").textContent =
        data.scores.achievements > 0
            ? "Achievements detected."
            : "Add measurable achievements.";

    document.getElementById("qualityFeedback").textContent =
        "Resume content quality analyzed.";

    document.getElementById("atsFeedback").textContent =
        "ATS compatibility evaluated.";
...
function updateInsights(data) {
    ...
    if (data.scores.projects >= 15) {
        ...
        recommendations.push(
            "Add project technologies, features and measurable results."
        );
    }
    ...
    if (data.scores.achievements > 0) {
        ...
        recommendations.push(
            "Add hackathons, awards, scholarships or measurable achievements."
        );
    }
    ...
    if (data.scores.ats >= 10) {
        ...
        recommendations.push(
            "Use clear section headings and job-relevant keywords."
        );
    }
    ...
    if (!recommendations.length) {
        ...
        recommendations.push(
            "Continue tailoring your resume for each target role."
        );
...
function getStoredResumes() {
    ...
    try {
        ...
        return JSON.parse(
            localStorage.getItem("resumeInsightData")
        ) || [];
...
function updateAdminDashboard() {
    ...
    const analyzed =
        records.filter(item => item.status === "Analyzed");


    document.getElementById("adminStudents").textContent =
        records.length;

    document.getElementById("adminUploaded").textContent =
        records.length;

    document.getElementById("adminAnalyzed").textContent =
        analyzed.length;
...
function renderAdminRows(records) {
    ...
    const empty =
        document.getElementById("adminRecentEmpty");

    const table =
        document.getElementById("adminRecentTable");

    const rows =
        document.getElementById("adminRows");
...
document.getElementById("clearDataBtn")
    .addEventListener("click", () => {
        ...
        const confirmClear =
            confirm(
                "Clear all resume analysis data?"
            );
